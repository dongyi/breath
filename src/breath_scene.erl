...
breath_scene:leave(Userid, Scenepid) -> gen_server:call(?MODULE, {}).
breath_scene:join(Userid, Scenepid) -> gen_server:call(?MODULE, {})  %返回一个pid


handle_call({broadcast, {Userid, Scenepid, Msg}}) ->
    ok.
    
handle_call({leave, {Userid, Scenepid}) ->
    broadcast(Userid, Scenepid, <<Userid ++ "is leave">>),
    ok.

handle_call({join, {Userid, Scenepid}) ->
    broadcast(Userid, Scenepid, <<Userid ++ "come in">>),
    ok.
