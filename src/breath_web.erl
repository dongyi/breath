...
wait(Req) ->
    ...
    breath_room:wait_finish(get_session(Req), self()).
...
handle_request(Req, "/login/") ->
    Post = Req:parse_post(),
	case breath_room:join(chat_util:get_parameter("nick", Post), Req:get(peer)) of
	    {ok, SessID} -> 
